<script lang="ts">
  ...
  async function resetMonthlyBills() {
    if (!confirm('Reset semua status tagihan menjadi belum dibayar untuk siklus bulan baru?')) return;
...
<div class="space-y-5">
  ...
  {#if isLoading}
    <div class="p-10 text-center font-mono text-xs text-[var(--color-ink-muted)]">Memuat tagihan...</div>
  {:else if bills.length === 0}
    <div class="p-10 text-center border border-dashed border-[var(--color-border)] rounded-md space-y-1">
      <p class="text-[var(--color-ink)] font-semibold text-sm">Belum Ada Tagihan Terdaftar</p>
    </div>
  {:else}
        ...
        <div class={`border rounded-md p-3.5 flex items-center justify-between gap-4 transition-colors ${
          ...
          <div class="flex items-center gap-3">
            <button
              ...
              aria-label="Toggle status bayar"
            >
            ...
            <div>
              ...
              <div class="text-xs font-mono text-[var(--color-ink-muted)] mt-0.5">
                ...
                {:else}
                  <span class="font-semibold text-[var(--color-ink-muted)]">BELUM DIBAYAR</span>
                {/if}
...
{#if showModal}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--color-paper)]/85 backdrop-blur-md">
    <div class="w-full max-w-md bg-[var(--color-paper-2)] border border-[var(--color-border)] rounded-md p-6 space-y-4 shadow-xl relative">
      ...
      <h2 class="text-base font-bold font-mono text-[var(--color-ink)]">{editingId ? 'Edit Tagihan' : 'Tambah Tagihan Baru'}</h2>
      <form on:submit|preventDefault={handleSubmit} class="space-y-3.5">
        <div>
          <label id="lbl-bill-name" for="inp-bill-name" class="block text-xs font-mono font-semibold text-[var(--color-ink-muted)] uppercase tracking-wider mb-1">Nama Tagihan</label>
          <input
            id="inp-bill-name"
            type="text"
            bind:value={name}
            placeholder="mis. Sewa Rumah / WiFi"
            required
        ...
        <div>
          <label id="lbl-bill-amount" for="inp-bill-amount" class="block text-xs font-mono font-semibold text-[var(--color-ink-muted)] uppercase tracking-wider mb-1">Nominal Bulanan (Rp)</label>
          <input
            id="inp-bill-amount"
            type="number"
            bind:value={amount}
        ...
        <div>
          <label id="lbl-bill-due" for="inp-bill-due" class="block text-xs font-mono font-semibold text-[var(--color-ink-muted)] uppercase tracking-wider mb-1">Tanggal Jatuh Tempo (1-31)</label>
          <input
            id="inp-bill-due"
            type="number"
            bind:value={dueDate}
        ...
        <div>
          <label id="lbl-bill-notes" for="inp-bill-notes" class="block text-xs font-mono font-semibold text-[var(--color-ink-muted)] uppercase tracking-wider mb-1">Catatan</label>
          <input
            id="inp-bill-notes"
            type="text"
            bind:value={notes}
